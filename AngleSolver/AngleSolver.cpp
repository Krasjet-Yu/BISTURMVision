...
void AngleSolver::solveAngles()
{
    ...
    if (distance > 5000)
	{
		PinHole_solver();  //只需要一个点即可得到转换角度
	}
	// Target is moderate, using PnP solver
	else
	{
		P4P_solver();  //P4P需要四个控制点，距离太远的话得到的点可能误差比较大
	}
}
...
void AngleSolver::PinHole_solver()
{
	...
	//去畸变后的比值
	double rxNew = (pnt.x - cx) / fx;  // X/Z
	double ryNew = (pnt.y - cy) / fy;  // Y/Z
...
void AngleSolver::getAngle(vector<Point2f>& contourPoints, Point2f centerPoint, ArmorType type, double& yaw, double& pitch, double& evaluateDistance)
{
...
void AngleSolver::getTvec(Mat &dstTVec)
{
    dstTVec = this->tVec;
}
