...
GxCamera::GxCamera()
{
    g_hDevice = NULL;                     ///< Device handle  设备句柄
...
GX_STATUS GxCamera::openDevice(const char* CameraSN)
{
    ...
    g_pDeviceSN = const_cast<char*>(CameraSN);    ///真正的用途是非const 转const!

    //Get device enumerated number
    status = GXUpdateDeviceList(&ui32DeviceNum, 1000);
    std::cout << "GxUpdatDeviceList Success..." << std::endl;
    ...
    stOpenParam.accessMode = GX_ACCESS_EXCLUSIVE;  // 设备访问方式。只读、控制、独占
    stOpenParam.openMode = GX_OPEN_SN;             // 设备开放模式。相机可以通过SN、IP、MAC打开
    stOpenParam.pszContent = g_pDeviceSN;          // SN：序列号  IP：IP号
    ...
    size_t nSize = 0;
    //Get string length of Vendor name 获取供应商名称的字符串长度
...
GX_STATUS GxCamera::acquisitionStart(cv::Mat* targetMatImg)
{
    ...
    //Set buffer quantity of acquisition queue 缓冲区队列大小
