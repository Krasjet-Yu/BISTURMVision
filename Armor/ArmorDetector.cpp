...
/**
 * @brief copy source image to srcImg of detector
 * @param Mat& src
 */
...
/**
 * @brief: load source image and set roi if roiMode is open and found target(color) in last frame.
 */
void ArmorDetector::PreprocessImg()
{
...
{
    //firstly, load and set srcImg
    this->PreprocessImg(); //globally srcImg and preprocess it into srcImg_binary

    //secondlly, reset detector before we findLights or matchArmors(clear lights and armors we found in the last frame and reset the state as LIGHTS_NOT_FOUND)
    resetDetector();
    ...
    if (state == LIGHTS_FOUND)
    {
        ...
        //if the state is ARMOR_FOUND(detector has matched suitable armor)
...
/**
 * @brief: get the vertices and type of target Armor for angle solver
 */ 
